...
import carte.Carte;
import carte.CarteDijkstra;
import carte.CaseCarte;
import carte.CaseCarteDijkstra;
import robots.Robot;
...
public class Dijkstra {
	...
	public Chemin plusCourtChemin(CaseCarte destination) {
		...
		CaseCarteDijkstra caseCourante = (CaseCarteDijkstra) carte.getCase(destination.getLigne(), destination.getColonne());
	...
	private void majVoisins(CaseCarteDijkstra caseCarte) {
		for (Direction dir : Direction.values()) {
			try {
				CaseCarteDijkstra voisin = (CaseCarteDijkstra) carte.getVoisin(caseCarte.getLigne(), caseCarte.getColonne(), dir);
				long poids = robot.tempsDeplacement(caseCarte, voisin, carte.getTailleCases());
				voisin.setPoids(poids);
				voisin.setPrec(dir);				
			}
